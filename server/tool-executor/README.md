...
> ｜ `../../docs/05-test-cases/test-M5.md`（`TC-U-M5-001` / `TC-C-M5-001` / `TC-I-M5-001` / `TC-I-M5-002` / `TC-I-M5-005`）
...
>
> 职责：M5 的查询执行与留痕层。**当前已建 F-23**（工具注册与权限检查）、**F-24**（查询执行与真实返回）。
> 边界：**F-25 落痕 / F-26 重试与暂停未开工**；F-24 **只返回不落 `EXT-01`**（信封显式标 `persisted=false`、`persist_by="F-25"`）。
...
| 文件 | 职责 | 状态 |
| ---- | ---- | ---- |
| `index.js` | F-23 本体：CFG-02 工具注册（登记/查询）、CFG-03 授权登记、**调用前权限判定**（判定链互斥、受限必带原因）；**F-24 编排层**：`describeTools` + `executeQuery`（先判权限→允许才发请求→映射为 EXT-01 口径信封 + 四要素） | ✅ F-23 / **F-24** 已建 2026-09-19 |
...
- **Q-10（未决）**：`EXT-01` 无承载「限制」的字段，F-24 的 `four_elements.limits[]` 目前只能随信封返回；转 `EXT-02 evidence` 时归入 `applicability_scope`。是否要在 EXT-01 增列尚未裁决（见 `schema.md` §12）。

## 反向清单

- `../api/index.js`（F-23 路由：`/api/tools`、`/api/tool-permissions`、`/api/tool-permission/check`、`/api/tool-permission/check-batch`；**F-24 路由：`/api/tool-descriptions`、`/api/query`（受限 → 403）**）
- `../agent-orchestrator`（后续：M3/M4 调用工具前先查权限；发起真实查询并消费四要素）
- `./mcp-client.js`（F-24 传输层，被 `./index.js` 引用）
- `../../docs/03-locks/schema.md` §12（Q-09、Q-10 登记处）｜`../../prototype/mock/scenarios.js`（夹具来源）
