...
>
> **本模块硬红线（与全局一致）**：生产零写（本目录只写 **MD-13/MD-14**（经 `profile.js`）、**MD-09/MD-10**（经 `behavior-store.js`）与 **MD-07 内容填充 / MD-08 / MD-11**（经 `result-store.js`）三处业务面，且各表只经本 README 列出的**唯一写入面**；`LNK-02 finding_evidence` 的写入不在本模块、归 `../shared-context/index.js`）、真实返回禁模型替代、证据四要素、失败不否定结论；外部依赖门禁（A-1）未关闭前，推理相关用例一律 mock/demo、demo 值不进断言。**F-22 起新增一处跨模块只读面**：`followup-intake.js` 经 `../task-runner/step-plan.js` 读 `PD-01 task`（M1 读面——阶段4 dev-plan「把真 Agent 接进 M1 的调度回路」），本模块**仍不含任何跨模块写动作**。
