...
- **本模块零外部调用**：`test-f01.mjs` 静态断言本目录不出现 `fetch(` / `http(s):`。
...
- **下游（我被谁引用）**：`../api/index.js`（F-01 路由）｜阶段4 `../agent-orchestrator`（消费任务与上下文）｜
  阶段5 `frontend/`（F-27 目标配置页经 `api` 取数）
  > 反向清单**只记既有事实**：本目录内 F-02~F-06 的引用关系在各自落地时补记（避免出现对未建文件的悬空引用）。
