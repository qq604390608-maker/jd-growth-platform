 ...
 * 职责：M1 恢复编排层。复用 F-26 `task-state.js` 单一写入面（PD-01/PD-03），本文件**不写任何 SQL**、**不发起任何外部调用**、
...
/**
 * 恢复（BRD F-06 状态流「检查继续条件满足 → 恢复原任务」）：仅 `blocked` 任务可恢复，置 `running`。
 * 受阻记录（`PD-03`）为**追加式历史缺口日志**，本函数不改动它——恢复是状态跃迁（可查），缺口记录仍留作审计。
 */
