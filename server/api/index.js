    ...
    try {
      ...
      if (pathname === "/api/ai-ping" && request.method === "GET") {
        try {
          ...
          return Response.json({ ai: "reachable", model: MODELS.FLASH, response: content.trim() });
        } catch (e) {
          return Response.json({ ai: "error", message: String(e) }, { status: 500 });
        }
      }

      // A-1 LLM function calling 测试：验证工具调用链路（一次简单 tool call）
      if (pathname === "/api/ai-tool-test" && request.method === "POST") {
        try {
          if (!env.AI) return Response.json({ ai: "not_bound" }, { status: 503 });
          const { chatWithTools, MODELS } = await import("../agent-orchestrator/llm-client.js");
          ...
          return Response.json({ ai: "tool_call_ok", ...result });
        } catch (e) {
