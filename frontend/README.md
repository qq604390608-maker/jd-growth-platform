...
2. **口径检查不自动去重** → **✅ 已收口（2026-09-21 裁决：去重）**：F-01 `checkGoalGaps` 已增加「同 goal 同规则已有 `is_solved=0` 待补项 → 跳过（`why=open_gap_exists`）」——重跑只新建检查任务与留痕，**不再落重复待补项行**（`test-f01` 88/0、`test-f27` ⑥ 组已翻为「重跑不重复列」130/0）。自动跑时机不变：进入页面仍不自动跑（偏差 ④）。
...
7. **机会状态字典无只读接口** → **✅ 已收口（2026-09-21）**：新增只读路由 `GET /api/dicts/{dict_type_code}`（值域真源 `dict_item`，只回启用项）；`app.js` 启动时经 `window.API.dict()` 拉 `OPP_STATUS` / `TASK_STATUS` 的中文口径**覆盖静态表文案**（`item_name`；视觉类 `cls` 仍归前端静态表）。取数失败时静默保留静态表作**展示兜底**、不阻断页面（徽标文案是展示副本而非业务判据，种子值与原静态表逐字一致，切换前后渲染不变——`test-f28` 已锁 `/api/dicts/` 调用与装配逻辑）。
8. **机会页首次加载取数 13 次** → **✅ 已收口（2026-09-21）：13 → 10 次**——新增批量读面 `GET /api/opportunity-readmodels?ids=…`（上限 100，`missing_ids` 显式回报不静默吞），本页逐条读模型 6 次并为 1 次；已提交研究仍按机会逐个取（面窄、量小）。外壳启动 3 → 5 次（新增字典 `OPP_STATUS` / `TASK_STATUS` 各 1 次，见缺口 7）。
...
12. **建议页取数 6 次** → **取数结构不变（2026-09-21 说明更新）**：外壳启动 3 次（目标列表 / 目标一页读 / 机会计数）+ 本页 3 次（机会列表 1 + 该机会读模型 1 + 建议列表 1）；提交动作后再 3 次。批量读模型端点已建（缺口 8），但本页**只读一条机会读模型**，无批读收益，故维持单条面；提交后的 3 次为动作后刷新，属必要事实回读。
