...
| ---- | ---- |
| 文档编号 | LOCK-TECH-001 |
| 版本 | **v1.5**（2026-09-21）｜ v1.0 同日首次建立；**v1.1 回填 D1 实测结论**（§3.1 类型与长度、§3.2 中文排序、§3.3 外键与 CHECK —— 对应 §8 的 **TS-11 / TS-14**）；**v1.2 将 §8 待确认项 `T-10`~`T-22` 重命名为 `TS-10`~`TS-22`**（引入 `TS-` 前缀以与 `external-deps.md` §7 的 `T-xx` 区分命名空间，消除 4 处撞号 T-10 / T-20 / T-21 / T-22），并承接 `schema.md` v1.2 三项裁决的连带同步更新；**v1.3 落地两项用户裁决（2026-09-20）**——① §2.5 模型要求由「必须支持 function calling」**放宽**为「须支持 JSON schema 结构化输出 + 敢于说『不足以判断』」（实施发现模型**并不参与工具选择**：F-19 已确定性编排排查证顺序、F-24 自建 MCP 客户端按 `tool_id` 执行，见 `server/tool-executor/mcp-client.js`；原约束过严且与代码实际不符，放宽后候选池显著扩大）；② §8 **TS-11** 裁定为**应用层校验**（`varchar(n)` 长度在 D1 一律不强制，事实面已于 v1.1 实测完成）。旧版归档 `.trash/tech-stack.md-v1.2.md`；**v1.4 关闭 TS-21**——§4 平台限制数字对照官方三页现值复核（2026-09-21）：一致 20+ 项，**修正 2 处**（Cron Trigger CPU 按〈1h 间隔 30 秒 / ≥1h 15 分钟〉、Queue Consumer CPU 默认 30 秒可提至 5 分钟——原记 15 分钟实为墙钟；Worker 体积现值未压缩 64 MiB 两计划相同）＋**补充 1 处**（Queues 消息保留期 Free 固定 24h 不可配）；三条设计结论不变。旧版归档 `.trash/tech-stack.md-v1.3.md`（SHA 7a4bf3396667）；**v1.5 关闭 TS-22**——依 ADR-004（无外部对接方，契约基准 v1 冻结）：基准 v1 下 mock 调用无凭证需求，T-04 权限边界已随基准自拟；真对接方出现时随基准 v2 重开。旧版归档 `.trash/tech-stack.md-v1.4.md`（SHA f2f7cd132d34） |
...
> 数字摘自 Cloudflare 官方 Limits 页，**查询日 2026-09-18**（时效声明见 §0.4）。**✅ TS-21 复核（2026-09-21，对照官方三页现值逐项核过）**：D1 全部 9 项、Queues 消息大小/批量/重试/吞吐/并发消费者等、内存/出网连接/子请求/Cron 条数均一致；**2 处修正**（Cron/Queue Consumer 的 CPU 数字、Worker 体积）＋ **1 处补充**（Queues 保留期 Free 计划固定 24h），见下表标注。
...
| 单行 / 字符串上限 | 2 MB | **约束**：`EXT-01 result_summary`、`EXT-02 result_summary` 若存大结果，须**截断或分段**。✅ **已落地（2026-09-21，TS-16 应用层截断）**：`server/tool-executor/text-limit.js`——单列上限 1,000,000 字节，超限在**写入面落库前**截断（字符边界）＋文末 `[TRUNCATED …]` 留痕标注；分析面仍拿全量原文 | F-09 F-12 F-24 F-25 |
...
| CPU 时间（Cron Trigger） | **间隔 <1 小时：30 秒；≥1 小时：15 分钟**（Paid）【TS-21 复核修正 2026-09-21，旧记 15 分钟不准确】 | runner Worker 是**每分钟 cron → CPU 默认 30 秒**：轮询逻辑须保持轻量（现实现仅查询＋判定＋建任务，满足；不得把重活塞进 scheduled） | TS-20 |
| CPU 时间（Queue Consumer） | 默认 30 秒，`limits.cpu_ms` **可提至 5 分钟**（Paid）【TS-21 复核修正 2026-09-21，同上】 | 单步消费的 **CPU** 预算＝5 分钟封顶；本平台步骤多为 I/O 等待（LLM/外部工具调用，等待不耗 CPU），CPU 面不紧 | F-06 F-24 |
| 消费者墙钟时长 | **15 分钟**（Cron 与 Queue Consumer 相同） | **这是「单次调用」的墙钟硬上限**：单步的等外部时间也计入，**外部系统响应慢（如活动报名系统“响应慢”）会直接吃掉预算**；`CFG-04 max_duration_min` 若指单次调用时长则受此约束 | F-06 F-24 F-26 |
...
| Worker 体积（未压缩 bundle） | **64 MiB（Free 与 Paid 相同）**【TS-21 复核修正 2026-09-21，旧记 Free 3 MB / Paid 10 MB 已过时】 | 体积极充裕；前端静态资源**不计入** Worker 体积（作为静态资产单独托管） | — |
...
| 消息保留期 | **Paid 可配最长 14 天；Free 固定 24 小时且不可配**【TS-21 补充 2026-09-21】 | 积压消息的存活窗口，超过即被删除；研发期若在 Free 计划上验 Queues 须注意 24h 窗口 | F-06 |
...
2. **单步任务墙钟 ≤ 15 分钟**（Cron 与 Queue Consumer 相同；CPU 另计：默认 30 秒 / 可提至 5 分钟）【TS-21 修正 2026-09-21】→ `CFG-04 max_duration_min` 不是自由配置，**其墙钟上限被平台钉死**。研究任务必须**切成多步**，这也正好与 `PD-02 task_step` 的粒度设计吻合。
...
| `max_duration_min` | Queues 消费者**墙钟**上限（CPU 另有独立预算） | **≤ 15 分钟**（墙钟平台硬上限；CPU 默认 30 秒 / `limits.cpu_ms` 可提至 5 分钟，I/O 等待不耗 CPU）【TS-21 修正 2026-09-21：原「CPU/墙钟 ≤15 分钟」混写，实为墙钟 15 分钟、CPU 30s~5min】。超过则任务必须切步 |
...
| TS-16 | 长文本（`result_summary` / 注入快照）的截断或分段策略（受 2 MB/行 限制） | 技术方 | F-09 F-12 F-24。**✅ 已决（2026-09-21，依用户裁决）：应用层截断＋留痕标注**——与 TS-11「应用层校验」同一取向，超限截断并标注真实原因，不引入对象存储/分段多行。**✅ 已落地（同日）**：`server/tool-executor/text-limit.js` 纯函数＋F-25 `saveQueryRecord`（EXT-01）与 F-09 `createEvidence`（EXT-02）两个写入面接入，单列上限 1,000,000 字节、字符边界截断、文末 `[TRUNCATED <列名>]` 留痕；用例 `test-ts16.mjs` 16 断言（含两写入面端到端）进 CI。**事实修正**：PD-06 `context_injection` 只存 `ref_object_id` 引用不存内容（见 DDL），不落大文本，从约束对象中移除 |
...
| TS-21 | 平台限制数字的复核（本文件 §4 的数摘于 2026-09-18） | 技术方 | 实施前须重核一次。**✅ 已复核（2026-09-21，TS-21 关闭）**：对照官方 D1/Queues/Workers 三页 Limits 现值逐项核对——一致 20+ 项；**修正 2 处**（Cron/Queue Consumer CPU 数字、Worker 体积 64 MiB）＋**补充 1 处**（Queues 保留期 Free 固定 24h），均已在 §4 原位标注；三条设计结论（必须 Paid / 墙钟切步 / 消息瘦状态厚）全部不变 |
| TS-22 | 外部系统凭证的获取与最小权限（承接 `external-deps.md` T-04） | 对接方 | ~~F-23 F-24 真实接入~~ **✅ 已收口（2026-09-21，ADR-004）**：无外部对接方，契约基准 v1 下 mock 调用**无凭证需求**（T-04 权限边界已随基准 v1 自拟冻结）；真对接方出现时随契约修订（基准 v2）重新打开本项 |
