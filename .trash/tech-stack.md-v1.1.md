...
| ---- | ---- |
| 文档编号 | LOCK-TECH-001 |
| 版本 | **v1.1**（2026-09-19）｜ v1.0 同日首次建立；**v1.1 回填 D1 实测结论**（§3.1 类型与长度、§3.2 中文排序、§3.3 外键与 CHECK —— 对应 §8 的 **T-11 / T-14**），并承接 `schema.md` v1.2 三项裁决（Q-03/Q-04/Q-05）在技术侧的连带。旧版归档 `.trash/tech-stack.md-v1.0.md` |
| 状态 | 草稿，待 PM 与技术方评审（**T-11 策略仍待裁决**，见 §8） |
| 上游约束 | `../../AGENTS.md`（宪法：一条硬红线 白盒原则｜编号体系｜双向引用｜术语口径）｜`../BRD.md`（F-01~F-32｜§2.2 系统形态｜§5.3 硬红线）｜`schema.md`（36 张表、§12 Q-01/Q-02；**v1.2 起含 Q-03/Q-04/Q-05 裁决**）｜`external-deps.md`（§4 交付基础设施 D-1~D-6、§7 T-21~T-27）｜`../07-decisions/ADR-001~003`（Q-03/04/05 裁决记录） |
...
>
> **两类证据的时效性不同（v1.1 补）**：§4 的数字来自**官方文档**（会随产品迭代变化，实施前须复核）；**§3 的结论来自 `--local` 实测**（属机制性行为，稳定性高，但**限于 miniflare / workerd 模拟**，不自动等同于线上 D1 —— 两份探针 README 均列了「未验证面」）。引用时**不要把两类混为一谈**：实测能证明「本机如此」，不能证明「线上必然如此」。
...
| ---- | ---- | ---- |
| `varchar(n)` | TEXT | **D1 不强制长度**（实测：`varchar(24)` 写 200 字符**不截断、不报错**，读回 `length()=200`、尾部完整）。`schema.md` 的 **180 个长度声明**只是文档约定，D1 完全不承担 → 兜底方式（应用层校验 or `CHECK`）**⬜ 仍待裁决**（§8 T-11） |
...
| 项 | 结论 |
| ---- | ---- |
| 外键（**T-14 已实测**） | `schema.md` §11 要求「MD/PD/CFG 之间一律建立真实外键约束」。**实测结论（2026-09-18，`--local`）**：D1 **默认强制外键**——`PRAGMA foreign_keys` 读回 `1`；插入违规子行或删除被引用父行均返回 `FOREIGN KEY constraint failed: SQLITE_CONSTRAINT_FOREIGNKEY`；且 **`PRAGMA foreign_keys=OFF` 无法关闭**（同 batch 内 OFF 后读回仍为 1，该 PRAGMA 被平台覆盖）。与官方文档「等价于每个事务都设 `foreign_keys=on`、用户查询无法改变」一致。**因此：无需在连接层每次 execute 前置开启，也无需写进迁移文件首行；`schema.md` §11 的真实外键可直接落。** 唯一合法放行手段是 `PRAGMA defer_foreign_keys=on`（**已实测**：事务内先违规后补父可成功且落库；事务结束仍未消解则整体回滚） |
...
| T-11 | `varchar(n)` 长度约束在 D1 的兜底策略（应用层校验 or `CHECK` 约束） | 技术方 | `db/` 迁移脚本写法。**实测已完成 2026-09-18**（`db/probes/type/`：长度**一律不强制**；`CHECK` 可用、按**字符**计、**不拦 NULL**）→ **事实面已齐，策略仍待裁决**（分类清单见该 README §5：编号锚点 94 / 长文本 39 / 其余短字段 47）。**阻塞面**：若裁决「库级 `CHECK`」须写进 `0001`；若裁决「应用层校验」则不影响首版迁移 |
| T-12 | 哪些列需要中文排序 → 是否增设拼音/排序键列 | PM + 前端 | F-27/F-28 列表排序。**部分实测已完成**：默认 **BINARY ≠ 拼音序**（已证实）、D1 **无拼音排序规则**（`COLLATE PINYIN` 报错）→ 仍待 PM 逐列确认「哪些列要排拼音」 |
| T-13 | 前端是否需要轻量组件复用手段（如 Web Components） | 前端 | `frontend/` 开发方式 |
| T-14 | **D1 中 `PRAGMA foreign_keys` 是否默认生效**（`schema.md` §11 承诺了真实外键，须实测） | 技术方 | `schema.md` §11 的可兑现性。**✅ 实测已完成 2026-09-18**（`db/probes/fk/`）：**默认强制外键**、`OFF` 无法关闭、临时放行只能 `defer_foreign_keys`；§11 可直接落。**限 `--local`；`--remote` 待补测**（见 §3.3 未验证面） |
...
| `../../db/probes/fk/README.md` | 其 T-14 实测由本文件 §3.3 + §8 T-14 采纳 | ✅ 已回填（2026-09-19） |
| `../../db/probes/collation/README.md` | §3.2 基础事实；其 §6 的「字典 `ORDER BY order_no` 零改动规避」尚未收录 | 🟡 部分已回填（2026-09-19），其余属 T-12 剩余 |
| `../../server/`（五个模块） | 实现依据 | ⏳ 待建 |
| `../../frontend/`（六页面） | 形态与部署依据 | ⏳ 待建 |
| `../../db/` 迁移脚本 | §3 是建表的唯一转换口径 | ⏳ 待建 |
| 开发计划 / 测试用例 / runbook | 引用平台限制与部署形态 | ⏳ 待建 |

> **诚实说明**：截至 **v1.1**（2026-09-19），本文件对三份探针的**事实回填已完成两处、部分一处**——`db/probes/type/README.md` 与 `db/probes/fk/README.md` 反向清单里的 `⏳ 待回填` 已分别由 §3.1/§3.2/§3.3 与 §3.3/§8 T-14 兑现；`db/probes/collation/README.md` 只**部分兑现**（§3.2 采纳了它的基础事实，「`ORDER BY order_no` 零改动规避」这条建议**尚未收录**，属 T-12 剩余）。反向引用中**代码级引用方**（`server/` `frontend/` `db/` 迁移、开发计划、测试用例、runbook）**仍未建立**。此外 §8 仍有 **13 条待确认**（其中 **T-11 策略待裁决**、**T-12 部分待确认**、**T-14 远程待补测**）、§4 的数字有时效性（§0.4）、`external-deps.md` §7 的 T-01~T-10 仍是本文件实现的前置。
