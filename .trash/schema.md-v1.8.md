...
| 版本 | **v1.8**（2026-09-21）｜ v1.0 同日首次建立；v1.1 落地宪法编号裁决（`R-xx` 归研究结果编号、宪法升 v1，见 §0.7），并按「双向引用」重写尾部引用卡（区分「已实际引用」与「预计引用」）；v1.2 落地 `DEC-PACK-001` 三项裁决 —— Q-03 不联动 / Q-04 维持现名 / Q-05 六要素必填用 `NULL`·空串二态（见 §12 状态与 `../07-decisions/ADR-001~003`）；**v1.3 纯交叉引用校正**——宪法升 **v2**（索引表状态列校正）后同步 §0.7 的版本表述，**表的数量 / 字段 / 类型 / 约束一律未动**，不改任何口径；**v1.4 收口 §12 待裁决项为已决** —— Q-09~Q-12 采纳方向②（不增 DDL/字段，维持现状兜底并测试锁死）、Q-13/Q-15 采纳方向①（种子对齐字典）、Q-14 五处失真中④已由实施侧回写、①②③⑤ 维持「以 PRD 功能点+DDL 为准」不擅改上游；并修 C4（§12 反向引用卡 `ADR-001~003` 补中文文件名后缀，消除真悬空）。**表的数量 / 字段 / 类型 / 约束一律未动**（Q-13/Q-15 仅改种子单文件），不改任何口径；**v1.5 登记 F-17 实施发现的新待裁决项 Q-16；**v1.6 收口 Q-16（采纳方向①：改种子把 linked_at 改写为机会 created_at 真实时点，不再写任务 ID；仅改 db/seed 单文件，DDL 不动）**** —— `LNK-01 opportunity_evidence.linked_at` 种子值形式与列定义（`datetime` 时点）不符（写的是产出任务 ID），F-17 按「只透传不解析」推进、未擅自改种子；**表的数量 / 字段 / 类型 / 约束一律未动**。旧版归档 `.trash/schema.md-v1.4.md`（v1.3 见 `.trash/schema.md-v1.3.md`，v1.2 见 `.trash/schema.md-v1.2.md`，v1.1 见 `.trash/schema.md-v1.1.md`）。**v1.7 登记 TS-20 双 Worker 落地发现的新待裁决项 Q-17**（CFG-04 无 last_run 列——runner 到期判定以 PD-01 推导 + 最小间隔近似，等用户追认；**表的数量 / 字段 / 类型 / 约束一律未动**）。旧版归档 `.trash/schema.md-v1.6.md`（SHA 3e0743b73670）。**v1.8 收口 Q-17**（依用户追认：采纳近似方向①——「PD-01 推导 + 最小间隔」到期判定口径定案，方向② CFG-04 增列否决留档；不加列、不改 DDL，**表的数量 / 字段 / 类型 / 约束一律未动**）。旧版归档 `.trash/schema.md-v1.7.md`（SHA 76665a6fad62） |
| 状态 | 草稿，待 PM 评审 |
| 上游约束 | `../../AGENTS.md`（白盒原则｜编号体系｜双向引用｜术语口径）｜`../01-brd/BRD.md`（M1–M6 × F-01~F-32）｜`../07-decisions/DEC-PACK-001.md`（§1.6 / §2.7 / §3.7 三项裁决；其升出的 ADR-001~003 为本次改动依据） | 
| 事实来源 | `../../prototype/assets/data.js`（全站唯一 mock 数据源：目标 / 机会 / 证据 / 研究 / 任务 / 追问）｜`../../prototype/assets/app.js`（跨页同口径的解析逻辑）｜`../../db/probes/type/`（D1 类型 / 长度 / 约束实测，2026-09-18 —— Q-05 二态写法的技术前提） |
...
| `opportunities[].unknown`（文本） | MD-06 `unknown_item`（**二态**，`N`→`Y`） | 原型只呈现「有未知项」**一态**（8 条机会的 `unknown` **全部非空**）；落库按 `ADR-003` 补上「未评估（`NULL`）」与「已评估且确无（`''`）」两态——**原型未覆盖这两态**，前端实现时须按二态处理，不能只判非空 |

### 0.7 与宪法编号体系的冲突（**已裁决：编号冲突于 v1/v2 收口；`TS-xx`/`T-xx`/`DS-xx`/`Q-xx` 前缀于 v3 补登记**）

`AGENTS.md` 现为 **v3**（2026-09-19；v1 于 2026-09-18 建立，v2 于 2026-09-19 校正索引状态列）。其关键规则载明：**编号体系**＝ M1–M6 模块 ｜ F-xx 功能点 ｜ `REQ-xx` 需求条目 ｜ `R-xx` 研究结果对外编号 ｜ `S-Ax/S-Bx` Skill ｜ PRD-Mx 子 PRD ｜ ADR-xxx 决策 ｜ `TS-xx` 技术栈待确认（tech-stack §8）｜ `T-xx` 外部依赖待确认（external-deps §7）｜ `DS-xx` 决策请示包 ｜ `Q-xx` 锁定文件待决议项（schema §12）。此前核对出的两处冲突均已裁决：
...
| **Skill 编号未对齐** | `S-Ax/S-Bx` | 原写自由文本 `clue-scan` / `hva-five-checks` 并作主键 | **已修正**：MD-14 增 `skill_no` 承载 `S-Ax/S-Bx` 并作主键，原型代码名降为 `skill_code`（UK）。映射已确认（§12 Q-07 已决 2026-09-19）：`clue-scan` → `S-A1`（M3 机会发现 Agent）、`hva-five-checks` → `S-B1`（M4 HVA 分析 Agent） |

> **另注（已裁决）**：`AGENTS.md` 原小节标题为「两条硬红线」，但正文只列出 1 条（白盒原则）。2026-09-18 裁决：**标题收敛为「一条硬红线」，与正文实际条数一致**，不补写第 2 条。宪法 **v1** 已落地。
> **v2 附注（2026-09-19）**：宪法 v2 只校正索引表「状态」列（`docs/`、`db/` 由「待建」改「🟡 部分」），**未改本节任何编号口径**，故本节裁决结论继续有效。
...
| `goal_id` | varchar(32) | N | 六要素·对应目标：该机会对应哪个目标 | FK → MD-01；`CHECK (length(trim(goal_id)) > 0)` 拒空串 | F-10 F-28 F-32 |
| `goal_version_no` | int | N | 六要素·对应目标（版本）：产出该机会时依据的目标版本号（快照，不随目标更新而变） | 逻辑关联 MD-02；`CHECK (goal_version_no >= 1)` | F-10 F-28 |
...
| `target_object` | varchar(300) | N | 六要素·涉及对象：人群、品类或旅程环节，如「人群：2026Q3 首次下单新客 ｜ 旅程环节：首单履约后 ~ 复购前」 | `CHECK (length(trim(target_object)) > 0)` 拒空串 | F-10 F-28 |
| `phenomenon` | text | N | 六要素·观察现象：观察到的事实，只写现象不下结论 | `CHECK (length(trim(phenomenon)) > 0)` 拒空串 | F-10 F-28 |
| `initial_basis_note` | text | N | 六要素·初步依据的文字说明（资料或查询的来源与时点）；结构化证据走 LNK-01 | `CHECK (length(trim(initial_basis_note)) > 0)` 拒空串 | F-10 F-28 |
| `research_reason` | text | N | 六要素·研究理由：为什么这个问题值得追问 | `CHECK (length(trim(research_reason)) > 0)` 拒空串 | F-10 F-28 |
| `unknown_item` | text | **Y** | 六要素·未知项（**二态**）：`NULL` = **未评估**（视为六要素不齐，触发待补）；`''`（空串）= **已评估且确无未知项**；非空文本 = 已评估且有未知项。**禁止编造未知项充数** | 二态取值；判定「齐全」时 `unknown_item IS NULL` 即不齐 | F-10 F-15 F-28 |
...
**Q-05 落地说明（2026-09-19，依据 `ADR-003`）**：六要素的「必填」由**两条约束配合**实现——① 六个字段保持 `NOT NULL`（**拦 `NULL`**）；② 各自加 `CHECK (length(trim(x)) > 0)`（**拦空串**）。**二者不可互相替代**：D1 实测证实 `length(NULL) > 0` 求值为 `NULL` → **CHECK 会放行 `NULL`**（见 `../../db/probes/type/README.md` §3 第 19 / 23 行），故只加 CHECK 拦不住 NULL，必须与 `NOT NULL` 并用。`unknown_item` 按裁决**不参与必填**：改可空二态（`NULL` = 未评估 / `''` = 已评估且确无）。「六要素齐全」的判定口径与应用层约定见 `ADR-003`。
...
> **背景可回溯性的边界（Q-03 裁决，2026-09-19，依据 `ADR-001`）**：本表只记「注入了哪几条对象」（`ref_object_type` + `ref_object_id`）与「何时注入」。若被注入的 `MD-04` 背景行此后被**就地修改**，本表取回的是**该行的当前内容**而非当时内容——**「研究启动时的背景内容」无法精确还原**，这是 Q-03 选择「不联动」时**明示接受的代价**（精确到「注入了哪几条」而非「那几条当时写的是什么」）。若将来需要内容级快照，须重新裁决，回滚代价见 `ADR-001`。
...
> **背景的版本口径（Q-03 裁决，2026-09-19，依据 `ADR-001`）**：`background` 类型注入的是 `MD-04 business_context` 的**当前生效条目**（`is_active = 1`）。**业务背景不做定版快照**——回答「研究启动时采用的是哪一版背景」以 `PD-06 context_injection` 的记录为准（记下注入了哪几条对象），**不承担「当时的内容是什么」的还原责任**。取舍与代价见 `ADR-001`。
...
| 机会的未知项 | `unknown_item` | `unknown`（**原型名，弃用**）、`to_verify`、`pending_item` |
...
### 8.3 次级命名（**已裁决：四条均维持现名**，2026-09-19，见 `ADR-002`）

> 裁决依据 `DEC-PACK-001` §2.7。四条**全部维持现名**，备选名标注「未采纳」。
> 四条备选名在全库的实例数**均为 0**（只作为备选被提及），故本次裁决**零改名动作**，不动 18 处跨文件引用（含 2 份已落盘探针证据）。

| 命名 | **当前取值（已裁决维持）** | 备选（**未采纳**） | 若将来改，影响面 |
| ---- | ---- | ---- | ---- |
| 研究建议表 | `research_proposal` | ~~`research_request`~~ 未采纳 | MD-12 + LNK-04 的 `proposal` 取值 |
| 追问消息表 | `followup_message` | ~~`research_dialogue`~~ 未采纳 | PD-07 + EXT-01 `message_id` |
| 口径检查任务类型 | `goal_check` | ~~`metric_check`~~ 未采纳 | CFG-08 dict:TASK_TYPE |
| 字典类型命名 | `dict_type` / `dict_item` | ~~`code_table` / `code_value`~~ 未采纳 | CFG-07/08 全库引用（18 处 / 5 文件） |

> **第 3 条保留前置条件（四条中唯一置信度「低」的一条）**：若后续确认「口径检查」的实际校验范围**只针对指标口径**，则须改判 `metric_check`，**且必须同改** `CFG-05.target_field` 的取值集合（当前走 `dict:GOAL_FIELD` 六字段，非仅指标）——**二者不可只改其一**，否则名实不符。该项确认不阻塞建表，登记于 `ADR-002`。
...
> **约束强度说明（2026-09-19，Q-05 落地后）**：本矩阵只表达「功能点由哪些表承载」，**不表达约束强度**。Q-05 落地使以下两行新增**库级约束**（表与承载关系均未变，故矩阵条目不动）：
> - **F-10 / F-28**：「六要素齐全」由 `MD-06` 的 `NOT NULL` + `CHECK(length(trim(x)) > 0)` 承载（六个字段拒 `NULL` 与空串）；且「齐全」的判定包含 `unknown_item` 的**评估状态**——`unknown_item IS NULL`（未评估）视为**不齐**。
> - **F-15**：「对现象的解释必须标明尚未验证的部分」由 `MD-06.unknown_item` 的**二态**承载——空串表示「已评估且确无未知项」，是**合法结论**，不强制编造未知项。
>
> 本注为「加注不改表」，依据 `ADR-003`；矩阵本身无增删。
...
| Q-03 | `business_context` 是否需要与 `goal_version` 建版本联动（背景也版本化） | **✅ 已决 2026-09-19**：**不联动**——`MD-04` 零结构变更；「研究启动时采用的是哪一版背景」以 `PD-06 context_injection` 记录为准，**不做定版快照**。口径文字已写入 CFG-06 / PD-06（依据 `DEC-PACK-001` §1.6、`ADR-001`） | 已落地，无待办 |
| Q-04 | §8.3 四条次级命名的最终取舍 | **✅ 已决 2026-09-19**：**四条全部维持现名**（`research_proposal` / `followup_message` / `goal_check` / `dict_type`+`dict_item`），备选名标注未采纳；**零改名动作**（依据 `DEC-PACK-001` §2.7、`ADR-002`）。第 3 条保留前置条件，见 §8.3 尾注 | 已落地，无待办 |
| Q-05 | 是否需要对 `opportunity` 的六要素做「必填」强制（当前 `unknown_item` 可为空说明） | **✅ 已决 2026-09-19**：**六要素必填，`unknown_item` 用 `NULL` / 空串二态**——`unknown_item` 由 `N` 改 `Y`（同时消解 `DEC-PACK-001` §5.1 记录的「§12 措辞 vs 表定义」矛盾，以本裁决为准）；其余六字段保持 `N` 并加 `CHECK (length(trim(x)) > 0)`；「六要素齐全」＝六项都被评估过（依据 `DEC-PACK-001` §3.7、`ADR-003`） | 已落地；DDL 见 `db/migrations/`（待建） |
...
| Q-07 | Skill 编号 `S-Ax/S-Bx` 与代码名的映射（`clue-scan` → `S-A1`、`hva-five-checks` → `S-B1`） | **✅ 已决 2026-09-19**：**映射确认**——`clue-scan` → `S-A1`（M3 机会发现 Agent 主用）、`hva-five-checks` → `S-B1`（M4 HVA 分析 Agent 主用）；MD-14 `skill_no` 主键据此赋值。原「开放」措辞中的「阻塞 MD-14 种子」随本裁决**解除** | 已落地（建表）；MD-14 种子待 `db/seed/` 补种（解除阻塞） |
| Q-08 | `PD-06.ref_object_type`（走 `dict:OBJECT_TYPE`，现四项＝`goal`/`opportunity`/`research`/`proposal`）**装不下三类上下文对象**——`background`（MD-04 背景条目）/ `source`（CFG-01 来源）/ `existing_evidence`（EXT-02 证据）在 `dict:OBJECT_TYPE` 无取值，而 `ADR-001` §3 第 3 条要求 PD-06「记下注入了哪几条对象」（背景尤甚） | ✅ **已决 2026-09-19（采纳方向②）**：维持 `dict:OBJECT_TYPE` 四项不变；`background` / `source` / `existing_evidence` 三类**只装配、不落 PD-06 行**，装配结果每段打 `no_object_type_q08` 显式标出（已写入 `test-f12.mjs` 断言，不留隐式缺口）。① 扩字典方案**不采纳**。`ADR-001` §3 第 3 条「记下注入了哪几条对象」在方向②下由**装配结果**（逐段 `items` 含对象 id）承担、由调用方（M3/M4 执行留痕）承接，**PD-06 不逐条落行**——这是本裁决承认的粒度让步 | 已落地（`server/shared-context/index.js` 的 `CONTEXT_OBJECT_TYPE` / `CONTEXT_WITHOUT_OBJECT_TYPE`）；若后续要求库内留痕，改该二表 + 补 `dict_item` 即可，无 DDL 变更 |
| Q-09 | `CFG-02 tool_registry` **无「存在性」字段**——只有 `is_enabled`（0/1），无法区分「存在但未接入」与「**根本不存在**」；而 `external-deps.md` §5 结论 1 明确要求「登记 TOL-12 的不存在，与登记工具本身同样重要」（否则 Agent 会把「查不到」误判为「没接通」） | ✅ **已决②（2026-09-20，依用户裁决）**：维持现状——以 `is_enabled=0` + `call_condition`/`tool_purpose` 文本表达「不存在」，判定链合并为 `tool_disabled`（见 `server/tool-executor/index.js` 的 `REASON`）；**不增 `existence` 字段**，DDL 不变 | 已落地；无待办 |
| Q-10 | `EXT-01 query_record` **无承载「限制」的字段**——该表只有 `restricted_flag`（0/1，表权限受限），装不下 F-24 从返回体萃取的一类软限制（来源降级、覆盖截断、空结果、不可算、多值口径、外部枚举未归一化，见 `external-deps.md` §6.8）；而这些限制是证据四要素之一（`BRD` §7 第 3 条要求返回带「限制」） | ✅ **已决②（2026-09-20，依用户裁决）**：限制只在 F-24 返回信封 `four_elements.limits[]` 承载，F-25 落痕时不写库（`readbackQuery` 返回 `limits_not_persisted: true`），转 `EXT-02 evidence` 时归 `applicability_scope`；**不增列**，DDL 不变 | 已落地；无待办 |
| Q-11 | `EXT-01 query_record.source_id` 为 **NOT NULL 外键**（→CFG-01），**装不下「工具/来源根本未登记」这类失败**——此时连「查的是哪个来源」都无从得知（F-23 判定链在 `tool_not_found` 处短路），而这正是 `PRD-M5` §4 约束 3「失败也留痕」要面对的一类结局 | ✅ **已决②（2026-09-20，依用户裁决）**：判定链短路即「未执行」，不改 `source_id` 可空；调用方以 `persist_skip` 显式标记「为何没留痕」，绝不静默丢（`saveQueryRecord` 无 `source_id` 抛 `PersistSkip`，`POST /api/query-records` 回 202）；DDL 不变 | 已落地；无待办 |
| Q-12 | 「**实在卡死保留状态停止**」**无可判定边界**——`BRD` §4 F-06 只写了「持续失败**暂停受影响研究**」（对应 `task_status=blocked`）与「真卡死**保留已完成部分停止**」（对应 `task_status=stopped`），受阻矩阵六行也未给「几次算卡死」的口径；而 `PD-01.task_status` 的 `blocked` / `stopped` 必须由代码确定性地选出，不能靠人读 | ✅ **已决①（2026-09-20，依用户裁决）**：同类受阻未解除即视为卡死 → `stopped`（对 `call_failed`/`source_unavailable` 同因适用、异因互不干扰）；口径已落地（`server/tool-executor/index.js` 的 `handleQueryFailure`），`test-f26.mjs` ④ 断言在案 | 已落地；若将来改连续 N 次/时间窗门槛需改 `CFG-04`+迁移+种子 |
| Q-13 | `CFG-05 gap_rule.target_field` 的**种子值域偏差**：`db/seed/0001_mock.sql` 里 GAP-3 写 `'scope'`、GAP-4 写 `'period'`，而 `dict:GOAL_FIELD` 只有 `business_scope` / `focus_period`（同旧账「种子 `LK-OR-001.relation_kind` 值域外」性质）。该列注释即 `-- dict:GOAL_FIELD`，故种子**违约** | ✅ **已决①（2026-09-20，依用户裁决）**：**改种子对齐字典**——`GAP-3` `'scope'`→`'business_scope'`、`GAP-4` `'period'`→`'focus_period'`；**同性质旧账 `LK-OR-001.relation_kind` `'related_update'`→`'superseded'`**（`dict:OPP_RELATION` 仅 `same_issue`/`superseded`，OPP-014 为 OPP-009 的后续精炼故取「被取代」）；消费方 `GOAL_FIELD_ALIAS` 别名映射保留为防御性兜底、不强制 | 已落地（种子已改）；`test-f01.mjs` ③ 仍断言落库值域合法 |
| Q-14 | **M1（阶段3）上游登记与引用失真五处**：① `dev-plan.md` 阶段 3「涉及 schema 表」清单**漏 `LNK-04 task_object`**（种子已有 11 行、服务 F-06/F-32，任务归属全靠它）；② `PRD-M1` F-01「关联 schema」**漏 `PD-01 task`**（而 `PD-04.raised_by_task_id` 是 NOT NULL FK→`task`，不建任务落不了待补项）；③ `test-M1.md` `TC-I-M1-002` 引用写「`schema.md` **PD-05**（UNIQUE `idempotency_key`）」——该约束实际在 **MD-12** `research_proposal`（行号 L302 正确、表号错）；④ `server/README.md` 模块清单 `task-runner` 行功能点写「F-02 F-04 F-05 F-06」**漏 F-01**（阶段3 覆盖 F-01~F-06）；⑤ `PRD-M1` F-03「关联 schema」**只列 `MD-12`、漏 `PD-05 opportunity_status_log`**——而 `test-M1.md` `TC-D-M1-007` 把 `PD-05` 的机会外键归给 **F-03**，种子 `LG-003`/`LG-004` 两条也写着「PM 提交研究建议（OPP-012 / OPP-010）」、正是 `candidate→submitted`（即 F-03 人工节点完成的那一步） | ✅ **已决（2026-09-20，依用户裁决）**：口径认可实施侧「以 `PRD-M1` 功能点范围为准 + 表结构以 DDL 为准」的推进；④ 已修（`server/README.md` 补 F-01）；①②③⑤ 的文字失真随 C 类旧账清理（dev-plan / PRD-M1 / test-M1 漏表 / 错表号）在 `decision-resolution` 一并修正 | ④ 已修；①②③⑤ 随 C1/C3 清理 |
| Q-15 | 种子 `T-1023`（`hva_followup`）的**追问链不自洽**：`parent_task_id='T-1021'`，但同一行的 `trigger_basis` 写「PM 于 2026-09-18 09:11 **在 R-007 上**提交追问」，且 `LNK-04` 的 `output` 关联是 `R-007`——而 `T-1021` 产出的是 `R-006`（`LK-TO-006`）。即「追问的原研究」与「`parent_task_id` 指向的原任务」对不上 | ✅ **已决①（2026-09-20，依用户裁决）**：**改种子对齐 F-05 规则**——`T-1023.parent_task_id` 由 `'T-1021'` 改为 `NULL`（因 `R-007.start_task_id=NULL`，Followup 的 parent 应＝原研究.start_task_id），`trigger_basis` 与 `LK-TO-004(output=R-007)` 维持，三者自洽；F-05 的 `origin_task_mismatch` 兜底保留为防御 | 已落地（种子已改） |
| Q-16 | `LNK-01 opportunity_evidence.linked_at` 的**种子值形式违约**：本文件该列定义 `datetime`「关联建立时点」（§LNK-01 字段表 L643），而 `db/seed/0001_mock.sql` 的 **10 行** `opportunity_evidence` 一律写**产出任务 ID**（`'T-1022'`/`'T-1018'`/`'T-1008'`），**不是时点**。该列**无 CHECK**，故种子照样载入（同 Q-13 性质：库级拦不住） | ✅ **已决①（2026-09-20，依用户裁决）**：**改种子把 linked_at 改写为机会 created_at 真实时点**（如 `'2026-09-16 00:00'`），不再写产出任务 ID；消费方 F-17 `assemblePmDecisionContext` 仍「只原样透传、不解析」（值现在本就是时点，语义对齐列定义）。**表的数量 / 字段 / 类型 / 约束一律未动**，仅改 `db/seed` 单文件 | 已落地（种子已改，linked_at 现为真实时点） |
| Q-17 | **CFG-04 `run_policy` 无 `last_run` / `next_run` 列**（2026-09-21，TS-20 双 Worker 落地发现）：runner Worker 的 `scheduled` 到期轮询需要「每个目标上次运行时刻」做判定依据，而 CFG-04 只有频率文本、无运行历史列；PD-01 `task` 可推导（`task_type='discovery' AND goal_id=?` 的 `MAX(started_at)`）但**含义是「任务创建时刻」非「策略名义触发时刻」** | ✅ **已决（2026-09-21，依用户追认）：采纳近似方向①**：不加列、不改 DDL——`server/task-runner/index.js` 的 `runDueDiscoveryCalls` 以「距上次 discovery 任务 ≥ 频率最小间隔」（daily=1440 / hourly=N*60 / weekly=10080 分钟）近似「到点触发」，实际触发时刻相对名义时刻**可漂移 ≤ 一个轮询周期**；若要求精确到点，方向② 为 CFG-04 增 `last_run_at` 列（触发 schema 变更流程）；真 Queues / per-policy cron 接入后可整体重审 | **已追认定案（2026-09-21）**：近似口径采纳，方向②（CFG-04 增列）否决留档；漂移只影响研究调度的触发时刻，不影响数据正确性；真 Queues / per-policy cron 接入后可整体重审 |
...
| `../01-brd/BRD.md` | M1–M6 × F-01~F-32 全量功能点 | 本文件「服务功能点」列的全部取值来源 |
| `../../prototype/assets/data.js` | 全站唯一 mock 数据源（目标 / 机会 / 证据 / 研究 / 任务 / 追问） | 字段口径的事实来源 |
| `../../prototype/assets/app.js` | 跨页同口径的解析逻辑 | 派生字段与状态流转的事实来源 |
| `../07-decisions/DEC-PACK-001.md` | §1.6（Q-03）/ §2.7（Q-04）/ §3.7（Q-05）三项裁决的底稿 | 本文件 v1.2 三项改动的**唯一依据** |
| `../07-decisions/ADR-001-业务背景不与目标版本联动.md` `ADR-002-次级命名维持现名.md` `ADR-003-机会六要素必填与未知项二态.md` | 三项裁决的正式决策记录（问题 / 备选 / 最终 / 权衡 / 影响面 / 回滚代价） | 落地细节与「将来反悔难度」的依据 |
| `../../db/probes/type/` | D1 类型 / 长度 / CHECK 的实测结论（2026-09-18） | Q-05 二态写法的技术前提、§0.3 类型列的方言约束 |
| `../../db/probes/fk/` | D1 外键默认强制性的实测结论（2026-09-18） | §11「真实外键」可兑现性的依据（TS-14），本文件不得改写其结论 |
...
| `db/` 建表迁移与全 mock 种子数据 | 本文件字段表是其字段清单的**唯一消费方** | ✅ **已建**（`db/migrations/0001_init.sql` 36 表 DDL；`db/seed/0001_mock.sql` 全量种子，`PRAGMA foreign_keys=ON` 校验 0 违例，2026-09-19） |
| 子 PRD（PRD-M1~M6） | 引用字段与口径 | ✅ 已建（2026-09-19，v1.0；M3/M4 同日订正 bump v1.1） |
| `../05-test-cases/`（测试用例） | 引用功能点与字段（白盒逐点验收、F-01~F-32 全覆盖） | ✅ **已建**（2026-09-19，86 条用例） |
| `tech-stack.md` | 承接本文件 §12 Q-01（表名前缀）、Q-02（数据库方言）的去处；并回填 D1 实测结论（TS-11 / TS-14） | ✅ **已建**（2026-09-20，v1.3） |
| `external-deps.md` | 与本文件 §7「不建表清单」互相印证；并承接 CFG-01 `capability_can/cannot` 的取值来源、CFG-02 工具种子清单、CFG-03 权限口径 | ✅ **已建**（2026-09-19，v1.1 —— 含 `OPP-015`→`OPP-011` 更正） |
| `../07-decisions/ADR-001~003` | 本文件 v1.2 三项改动的**决策记录**；§12 三行已回指 | ✅ **已建**（2026-09-19） |
| `../../db/probes/type/README.md` | 本文件 MD-06 的 `CHECK` 写法以其实测结论为依据 | ✅ **已建**（2026-09-18） |
| `../../db/probes/fk/README.md` | 本文件 §11「真实外键」可兑现性以其实测结论为依据（TS-14：D1 默认强制外键） | ✅ **已建**（2026-09-19 补登，实测于 2026-09-18） |

**同批锁定**：`external-deps.md`（外部依赖）、`tech-stack.md`（技术栈）——本文件与二者共同构成「三项锁定」，三者互为印证；边界划分见 `README.md`「三项锁定的边界」。

> **诚实说明**：截至 **v1.3**（2026-09-19），本文件的反向引用仍**只有「登记层」落地**（两份 README）与**四类已建下游**（`tech-stack.md` / `external-deps.md` / `ADR-001~003` / `db/probes/` 的 `type` 与 `fk` 两份探针）；标 ⏳ 的**代码级引用方**（`db/migrations/`、子 PRD、开发计划、测试用例）**均尚未建立**。待其建立时，须在各自头部上游卡中回指本文件，届时本表同步去 ⏳。此状态如实标注，不作「已完成双向引用」论。
>
> **v1.3 说明**：本次为**纯交叉引用校正**（宪法升 v2 → 同步 §0.7 的版本表述），表清单 / 字段 / 类型 / 约束 / §12 状态**均未变动**，故若下游已按 v1.2 建表，**无需因 v1.3 返工**。
