...
| ---- | ---- |
| 文档编号 | LOCK-TECH-001 |
| 版本 | **v1.2**（2026-09-19）｜ v1.0 同日首次建立；**v1.1 回填 D1 实测结论**（§3.1 类型与长度、§3.2 中文排序、§3.3 外键与 CHECK —— 对应 §8 的 **TS-11 / TS-14**）；**v1.2 将 §8 待确认项 `T-10`~`T-22` 重命名为 `TS-10`~`TS-22`**（引入 `TS-` 前缀以与 `external-deps.md` §7 的 `T-xx` 区分命名空间，消除 4 处撞号 T-10 / T-20 / T-21 / T-22），并承接 `schema.md` v1.2 三项裁决的连带同步更新。旧版归档 `.trash/tech-stack.md-v1.1.md` |
| 状态 | 草稿，待 PM 与技术方评审（**TS-11 策略仍待裁决**，见 §8） |
...
| ---- | ---- | ---- |
| `varchar(n)` | TEXT | **D1 不强制长度**（实测：`varchar(24)` 写 200 字符**不截断、不报错**，读回 `length()=200`、尾部完整）。`schema.md` 的 **180 个长度声明**只是文档约定，D1 完全不承担 → 兜底方式（应用层校验 or `CHECK`）**⬜ 仍待裁决**（§8 TS-11） |
...
| 编号 | 待确认事项 | 谁提供 | 阻塞什么 |
| ---- | ---- | ---- | ---- |
| TS-10 | **Workers AI 具体模型选型**：F-20 五查需强推理且须「敢说不足以判断」，须实机验证 | 技术方 + PM | 推理质量与「不编造」红线的达成 |
| TS-11 | `varchar(n)` 长度约束在 D1 的兜底策略（应用层校验 or `CHECK` 约束） | 技术方 | `db/` 迁移脚本写法。**实测已完成 2026-09-18**（`db/probes/type/`：长度**一律不强制**；`CHECK` 可用、按**字符**计、**不拦 NULL**）→ **事实面已齐，策略仍待裁决**（分类清单见该 README §5：编号锚点 94 / 长文本 39 / 其余短字段 47）。**阻塞面**：若裁决「库级 `CHECK`」须写进 `0001`；若裁决「应用层校验」则不影响首版迁移 |
...
> **诚实说明**：截至 **v1.2**（2026-09-19），本文件对三份探针的**事实回填已完成两处、部分一处**——`db/probes/type/README.md` 与 `db/probes/fk/README.md` 反向清单里的 `⏳ 待回填` 已分别由 §3.1/§3.2/§3.3 与 §3.3/§8 TS-14 兑现；`db/probes/collation/README.md` 只**部分兑现**（§3.2 采纳了它的基础事实，「`ORDER BY order_no` 零改动规避」这条建议**尚未收录**，属 TS-12 剩余）。反向引用中**代码级引用方**：`db/` 迁移 ✅ 已建、开发计划 ✅ 已建、测试用例 ✅ 已建（2026-09-19）；`server/` 🟡 工程骨架已建（2026-09-19）、`frontend/` 与 runbook 仍未建立。此外 §8 仍有 **13 条待确认**（其中 **TS-11 策略待裁决**、**TS-12 部分待确认**、**TS-14 远程待补测**）、§4 的数字有时效性（§0.4）、`external-deps.md` §7 的 T-01~T-10 仍是本文件实现的前置。
