...
> **版本：v2**（2026-09-19）。改版须 bump 版本号，旧版移入 `.trash/`，不直接删除。
>
> v2 变更：索引表「状态」列按**实际磁盘状态**校正——`docs/` 与 `db/` 由「待建」改为「🟡 部分」（两者均已有产物，原状态失实；该缺陷曾登记于 `docs/07-decisions/DEC-PACK-001.md` §5.3 与 `db/README.md` 已知缺口 1，均标「未擅自改宪法」，本次收口）。旧版见 `.trash/AGENTS.md-v1.md`。
...
| `db/`            | 建表迁移与全 mock 种子数据                                                            | 🟡 部分：`probes/` 已有；`migrations/`、`seed/` 待建 |
