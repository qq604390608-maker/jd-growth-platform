...
| 版本 | **v1.5**（2026-09-20）｜ v1.0 同日首次建立；v1.1 落地宪法编号裁决（`R-xx` 归研究结果编号、宪法升 v1，见 §0.7），并按「双向引用」重写尾部引用卡（区分「已实际引用」与「预计引用」）；v1.2 落地 `DEC-PACK-001` 三项裁决 —— Q-03 不联动 / Q-04 维持现名 / Q-05 六要素必填用 `NULL`·空串二态（见 §12 状态与 `../07-decisions/ADR-001~003`）；**v1.3 纯交叉引用校正**——宪法升 **v2**（索引表状态列校正）后同步 §0.7 的版本表述，**表的数量 / 字段 / 类型 / 约束一律未动**，不改任何口径；**v1.4 收口 §12 待裁决项为已决** —— Q-09~Q-12 采纳方向②（不增 DDL/字段，维持现状兜底并测试锁死）、Q-13/Q-15 采纳方向①（种子对齐字典）、Q-14 五处失真中④已由实施侧回写、①②③⑤ 维持「以 PRD 功能点+DDL 为准」不擅改上游；并修 C4（§12 反向引用卡 `ADR-001~003` 补中文文件名后缀，消除真悬空）。**表的数量 / 字段 / 类型 / 约束一律未动**（Q-13/Q-15 仅改种子单文件），不改任何口径；**v1.5 登记 F-17 实施发现的新待裁决项 Q-16** —— `LNK-01 opportunity_evidence.linked_at` 种子值形式与列定义（`datetime` 时点）不符（写的是产出任务 ID），F-17 按「只透传不解析」推进、未擅自改种子；**表的数量 / 字段 / 类型 / 约束一律未动**。旧版归档 `.trash/schema.md-v1.4.md`（v1.3 见 `.trash/schema.md-v1.3.md`，v1.2 见 `.trash/schema.md-v1.2.md`，v1.1 见 `.trash/schema.md-v1.1.md`） |
| 状态 | 草稿，待 PM 评审 |
...
| Q-16 | `LNK-01 opportunity_evidence.linked_at` 的**种子值形式违约**：本文件该列定义 `datetime`「关联建立时点」（§LNK-01 字段表 L643），而 `db/seed/0001_mock.sql` 的 **10 行** `opportunity_evidence` 一律写**产出任务 ID**（`'T-1022'`/`'T-1018'`/`'T-1008'`），**不是时点**。该列**无 CHECK**，故种子照样载入（同 Q-13 性质：库级拦不住） | ⬜ **待裁决（2026-09-20 由 F-17 实施发现并登记，未擅自改种子）**：两个方向——① **改种子**（`linked_at` 改写为真实时点串，如 `'2026-09-16 00:00'`，与该机会的证据取数时点对齐）；② **保持种子不动**，消费方**只原样透传、不解析**（**当前实现按 ②**：F-17 的 `assemblePmDecisionContext` 只把 `linked_at` 原值带出，不当作时点参与任何判定或排序）。本项**只影响种子数据、不影响 DDL** | 待 PM / 我裁决；按 ① 需改种子单文件 |
