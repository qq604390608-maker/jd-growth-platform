...
| Q-17 | **CFG-04 `run_policy` 无 `last_run` / `next_run` 列**（2026-09-21，TS-20 双 Worker 落地发现）：runner Worker 的 `scheduled` 到期轮询需要「每个目标上次运行时刻」做判定依据，而 CFG-04 只有频率文本、无运行历史列；PD-01 `task` 可推导（`task_type='discovery' AND goal_id=?` 的 `MAX(started_at)`）但**含义是「任务创建时刻」非「策略名义触发时刻」** | 🟡 **当前按近似方向实现（等用户追认）**：不加列、不改 DDL——`server/task-runner/index.js` 的 `runDueDiscoveryCalls` 以「距上次 discovery 任务 ≥ 频率最小间隔」（daily=1440 / hourly=N*60 / weekly=10080 分钟）近似「到点触发」，实际触发时刻相对名义时刻**可漂移 ≤ 一个轮询周期**；若要求精确到点，方向② 为 CFG-04 增 `last_run_at` 列（触发 schema 变更流程）；真 Queues / per-policy cron 接入后可整体重审 | 待用户追认近似口径（不阻塞：漂移只影响研究调度的触发时刻，不影响数据正确性） |
