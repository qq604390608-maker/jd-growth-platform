...
| 版本 | **v1.2**（2026-09-19）｜ v1.0 同日首次建立；v1.1 落地宪法编号裁决（`R-xx` 归研究结果编号、宪法升 v1，见 §0.7），并按「双向引用」重写尾部引用卡（区分「已实际引用」与「预计引用」）；**v1.2 落地 `DEC-PACK-001` 三项裁决 —— Q-03 不联动 / Q-04 维持现名 / Q-05 六要素必填用 `NULL`·空串二态**（见 §12 状态与 `../07-decisions/ADR-001~003`）。旧版归档 `.trash/schema.md-v1.1.md` |
| 状态 | 草稿，待 PM 评审 |
| 上游约束 | `../../AGENTS.md`（白盒原则｜编号体系｜双向引用｜术语口径）｜`../BRD.md`（M1–M6 × F-01~F-32）｜`../07-decisions/DEC-PACK-001.md`（§1.6 / §2.7 / §3.7 三项裁决；其升出的 ADR-001~003 为本次改动依据） | 
...
### 0.7 与宪法编号体系的冲突（**已裁决，宪法已升 v2**）

`AGENTS.md` 现为 **v2**（2026-09-19；v1 于 2026-09-18 建立）。其关键规则载明：**编号体系**＝ M1–M6 模块 ｜ F-xx 功能点 ｜ `REQ-xx` 需求条目 ｜ `R-xx` 研究结果对外编号 ｜ `S-Ax/S-Bx` Skill ｜ PRD-Mx 子 PRD ｜ ADR-xxx 决策。此前核对出的两处冲突均已裁决：
...
| Q-07 | Skill 编号 `S-Ax/S-Bx` 与代码名的映射（`clue-scan` → `S-A1`、`hva-five-checks` → `S-B1`） | 未决 | PM 评审。**开放**：不阻塞建表（`MD-14` 表结构已定），但**阻塞 `MD-14` 的种子数据**（映射未定则 `skill_no` 无法赋值） |
...
| `../../db/probes/fk/` | D1 外键默认强制性的实测结论（2026-09-18） | §11「真实外键」可兑现性的依据（T-14），本文件不得改写其结论 |
...
| `db/` 建表迁移与全 mock 种子数据 | 本文件字段表是其字段清单的**唯一消费方** | ⏳ 待建（`db/migrations/` 尚未创建；`db/probes/` 是探针证据，**非**业务 migration） |
| 子 PRD（PRD-M1~M6） | 引用字段与口径 | ⏳ 待建 |
| 开发计划、测试用例 | 引用功能点与字段 | ⏳ 待建 |
| `tech-stack.md` | 承接本文件 §12 Q-01（表名前缀）、Q-02（数据库方言）的去处；并回填 D1 实测结论（T-11 / T-14） | ✅ **已建**（2026-09-19，v1.1） |
...
| `../../db/probes/fk/README.md` | 本文件 §11「真实外键」可兑现性以其实测结论为依据（T-14：D1 默认强制外键） | ✅ **已建**（2026-09-19 补登，实测于 2026-09-18） |

**同批锁定**：`external-deps.md`（外部依赖）、`tech-stack.md`（技术栈）——本文件与二者共同构成「三项锁定」，三者互为印证；边界划分见 `README.md`「三项锁定的边界」。

> **诚实说明**：截至 **v1.2**（2026-09-19），本文件的反向引用仍**只有「登记层」落地**（两份 README）与**四类已建下游**（`tech-stack.md` / `external-deps.md` / `ADR-001~003` / `db/probes/` 的 `type` 与 `fk` 两份探针）；标 ⏳ 的**代码级引用方**（`db/migrations/`、子 PRD、开发计划、测试用例）**均尚未建立**。待其建立时，须在各自头部上游卡中回指本文件，届时本表同步去 ⏳。此状态如实标注，不作「已完成双向引用」论。
