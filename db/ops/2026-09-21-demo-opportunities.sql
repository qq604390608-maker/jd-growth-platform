...
-- 口径对齐（二次执行，2026-09-21）：查询条件/证据的 metric 已改为**逐字取自已应用版本 GOAL-2026Q3-01-v3**（`（已剔除退款与取消订单）` 结尾，**不含**「单品首单不计入」）。原先误用了 v2 的写法——v2（`is_applied=0`）含 GAP-001 补充句「单品首单不计入」，而 **v3（当前 `is_applied=1`）不含该句**。演示件只引用「已应用版本」，故按 v3 修正。v2/v3 之间的口径差异本身是一次已登记的观察（是否属于「补充未并入新版本」，见演示页 §6 与交付说明），不在本文件处理。
...
VALUES
 ('Q-00132', 'T-0021', 'CDP',
  '{"goal":"提升粮油调味品类新客 90 天复购率","metric":"粮油调味新客 = 考察期内首次购买粮油调味类目的用户；90 天复购 = 首单后 90 个自然日内再次下单（已剔除退款与取消订单）","period":"2026-04-01 ~ 2026-06-30","scope":"京东超市主站频道（APP + 小程序）；品类限粮油调味"}',
  '2026-09-21 13:45', 'ok',
