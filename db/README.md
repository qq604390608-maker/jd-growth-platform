...
> 宪法 `AGENTS.md` 索引给 `db/` 的职责：**建表迁移与全 mock 种子数据 ＋ 线上一次性运维 SQL（`db/ops/`）**（v4 起补入 `db/ops/`）。
...
| 上游 | 内容 | 约束力 |
| ---- | ---- | ---- |
| `../AGENTS.md` | 宪法 **v4**：`db/` 职责＝建表迁移与全 mock 种子数据 ＋ **线上一次性运维 SQL（`db/ops/`，经 CI 执行）**；双向引用；索引三层 | 强约束。**v2/v3 已把索引表 `db/` 行由「待建」经「🟡 部分」校正为「✅ 已建」**（2026-09-19）；**v4 状态列收敛为「状态 + 差额 + 指向真源」**（逐点明细不再写在宪法里，见 `docs/04-plan/dev-plan.md` 与本 README），本 README 目录清单同步 |
