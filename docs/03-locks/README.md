...
| `tech-stack.md` | 技术栈锁定：Cloudflare + GitHub 架构 · `server/` 五模块选型 · D1 落地约束（方言/字符集/排序/类型映射）· 平台限制→设计约束 · `CFG-04`→平台配置映射 | ✅ 已有（**v1.5** 草稿，2026-09-21 —— v1.3 含 TS-11 / TS-14 实测回填，TS-11 裁定为应用层校验、§2.5 模型约束放宽为「结构化输出 + 敢说不足以判断」；v1.4 关闭 TS-21（§4 平台限制数字对照官方现值复核）；**v1.5 关闭 TS-22**（依 ADR-004 契约基准 v1 冻结）） |
