...
| 文件 | 职责 | 状态 |
| ---- | ---- | ---- |
| `schema.md` | 数据库表结构锁定：**37 张表**（**业务表 36 张** ＋ 1 张运行期基础设施计数器表）的字段 / 类型 / 可空 / 口径 / 值域 / 服务功能点 | ✅ 已有（**v1.9** 草稿，2026-09-22 —— v1.2 含 Q-03/Q-04/Q-05 裁决落地；v1.3 为宪法升 v2 后的交叉引用校正；v1.4~v1.8 收口 Q-09~Q-17；**v1.9 落地 F-35 取号原子化**：新增 **CFG-09 `id_sequence`**（**运行期基础设施**），表数 **36 → 37**，**业务表结构未动**，登记 §12 **Q-18**） |
| `external-deps.md` | 外部依赖锁定：依赖总表（数据源/Agent 运行能力/交付基础设施三类）· 五系统能力核对七项 · `tool_registry` 文档视图（含 `blocks_features`）· mock server 需求 | ✅ 已有（**v1.4**（2026-09-22）；**v1.3 契约基准 v1 已冻结**（2026-09-21，ADR-004）：§7 全部 21 条收口，11 条自拟定案；真对接方出现时走 ADR 修订替换。v1.4 只同步 `schema.md` v1.9 的表数：§4 D-1「36 张表」→ **37 张**（业务表仍 36），契约基准 v1 与 §7 结论未变；旧版归档 `../../.trash/external-deps.md-v1.3.md`，SHA f19e660bcf45） |
| `tech-stack.md` | 技术栈锁定：Cloudflare + GitHub 架构 · `server/` 五模块选型 · D1 落地约束（方言/字符集/排序/类型映射）· 平台限制→设计约束 · `CFG-04`→平台配置映射 | ✅ 已有（**v1.5** 草稿，2026-09-21 —— v1.3 含 TS-11 / TS-14 实测回填，TS-11 裁定为应用层校验、§2.5 模型约束放宽为「结构化输出 + 敢说不足以判断」；v1.4 关闭 TS-21（§4 平台限制数字对照官方现值复核）；**v1.5 关闭 TS-22**（依 ADR-004 契约基准 v1 冻结）；**v1.6（2026-09-22）只同步 `schema.md` v1.9 的表数**：§0.2 DS-01/DS-04 论据、§1 架构图、§2.3 表数行、§12 引用卡四处「36 张表」→ **37 张**（业务表仍 36），选型与平台限制结论未变；旧版归档 `../../.trash/tech-stack.md-v1.5.md`，SHA 9b418af15732） |
