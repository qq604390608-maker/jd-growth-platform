...
| Q-09 | `CFG-02 tool_registry` **无「存在性」字段**——只有 `is_enabled`（0/1），无法区分「存在但未接入」与「**根本不存在**」；而 `external-deps.md` §5 结论 1 明确要求「登记 TOL-12 的不存在，与登记工具本身同样重要」（否则 Agent 会把「查不到」误判为「没接通」） | ⬜ **待裁决（2026-09-19 由 F-23 实施发现并登记，未擅自改口径）**：两个方向——① 增 `existence` 字段（如 `exists`/`not_exists`，或走 `dict:TOOL_EXISTENCE`）；② **不加字段**，以 `is_enabled=0` + `call_condition`/`tool_purpose` 文本表达不存在。**当前实现按 ②**（判定链把二者合并为 `tool_disabled`，见 `server/tool-executor/index.js` 的 `REASON`） | 待 PM / 我裁决；裁决后改一张映射表或加一个字段，DDL 变更范围限于 `CFG-02` |
| Q-10 | `EXT-01 query_record` **无承载「限制」的字段**——该表只有 `restricted_flag`（0/1，表权限受限），装不下 F-24 从返回体萃取的一类软限制（来源降级、覆盖截断、空结果、不可算、多值口径、外部枚举未归一化，见 `external-deps.md` §6.8）；而这些限制是证据四要素之一（`BRD` §7 第 3 条要求返回带「限制」） | ⬜ **待裁决（2026-09-19 由 F-24 实施发现并登记，未擅自改口径）**：两个方向——① EXT-01 增列（如 `limit_note text`）；② **不加字段**，F-25 落 `EXT-01` 时把限制并入 `result_summary` 或仅在转 `EXT-02 evidence` 时落 `applicability_scope`。**当前实现按 ②**（F-24 只在返回信封的 `four_elements.limits[]` 里承载，见 `server/tool-executor/mcp-client.js` 的 `collectLimits`；**F-25 落痕时不写库**——`readbackQuery` 显式返回 `limits_not_persisted: true`，限制仍只存在于 F-24 信封，转 `EXT-02 evidence` 时归 `applicability_scope`） | 待 PM / 我裁决；裁决后按 ① 需改 DDL + 迁移 + 种子三处，按 ② 无需变更 |
| Q-11 | `EXT-01 query_record.source_id` 为 **NOT NULL 外键**（→CFG-01），**装不下「工具/来源根本未登记」这类失败**——此时连「查的是哪个来源」都无从得知（F-23 判定链在 `tool_not_found` 处短路），而这正是 `PRD-M5` §4 约束 3「失败也留痕」要面对的一类结局 | ⬜ **待裁决（2026-09-19 由 F-25 实施发现并登记，未擅自改口径）**：两个方向——① 为「被事前判定拦下的尝试」也留痕（新增独立表，或放开 `source_id` 可空 + `CHECK` 二态）；② **不为未发生的执行留痕**（判定链短路时**根本没有执行**，不落入「每次**执行**把条件/来源/时点/结果或原因写入」的口径），改由调用方以 `persist_skip` 显式标记「为何没留痕」，**绝不静默丢**。**当前实现按 ②**（`saveQueryRecord` 无 `source_id` 时抛 `PersistSkip`，`recordQuery` 返回 `persist_skip.reason_code`，`POST /api/query-records` 回 202 而非 201；见 `server/tool-executor/index.js` 与 `test-f25.mjs` ④） | 待 PM / 我裁决；裁决后按 ① 需改 DDL + 迁移 + 种子三处，按 ② 无需变更 |
| Q-12 | 「**实在卡死保留状态停止**」**无可判定边界**——`BRD` §4 F-06 只写了「持续失败**暂停受影响研究**」（对应 `task_status=blocked`）与「真卡死**保留已完成部分停止**」（对应 `task_status=stopped`），受阻矩阵六行也未给「几次算卡死」的口径；而 `PD-01.task_status` 的 `blocked` / `stopped` 必须由代码确定性地选出，不能靠人读 | ⬜ **待裁决（2026-09-19 由 F-26 实施发现并登记，未擅自改口径）**：两个方向——① **同类受阻未解除即视为卡死** → `stopped`（**当前实现按 ①**：该任务在 `PD-03` 里存在 `is_resolved=0` 且 `block_reason_code` 与本次相同的记录 → `stopped`，否则 → `blocked`；对失败 `call_failed` 与受限 `source_unavailable` **同样适用**，两类原因不同码故互不干扰）；② 另立可判定门槛（如连续 N 次失败、时间窗、或直接以 `tech-stack` §4.2 的「超限进死信」为判据），需新增字段或策略项 | 已落地（`server/tool-executor/index.js` 的 `handleQueryFailure`；不变量见 `server/tool-executor/README.md`「卡死门槛」）；`test-f26.mjs` ④ 断言在案（含种子 `T-1020` 已受阻且 `BL-001` 未解除 → 再失败直接 `stopped`）；裁决后按 ② 需改 `CFG-04` + 迁移 + 种子 |
| Q-13 | `CFG-05 gap_rule.target_field` 的**种子值域偏差**：`db/seed/0001_mock.sql` 里 GAP-3 写 `'scope'`、GAP-4 写 `'period'`，而 `dict:GOAL_FIELD` 只有 `business_scope` / `focus_period`（同旧账「种子 `LK-OR-001.relation_kind` 值域外」性质）。该列注释即 `-- dict:GOAL_FIELD`，故种子**违约** | ⬜ **待裁决（2026-09-19 由 F-01 实施发现并登记，未擅自改种子）**：两个方向——① **改种子**（`'scope'`→`'business_scope'`、`'period'`→`'focus_period'`），与字典对齐，别名映射随之退役；② **保持种子不动**，由消费方做显式别名归一（**当前实现按 ②**：`server/task-runner/goal.js` 的 `GOAL_FIELD_ALIAS` 只映射已知别名、未登记名字一律抛错，映射结果才落 `PD-04.target_field` 保证值域合法，并在返回值里带 `alias_fields` 让偏差可见）；本项**只影响种子数据、不影响 DDL** | 已落地（`server/task-runner/goal.js`；`test-f01.mjs` ③ 断言在案：别名归一可见 + 落库值域合法 + 未登记名字抛错）；裁决后按 ① 需改种子单文件 |
| Q-14 | **M1（阶段3）上游登记与引用失真五处**：① `dev-plan.md` 阶段 3「涉及 schema 表」清单**漏 `LNK-04 task_object`**（种子已有 11 行、服务 F-06/F-32，任务归属全靠它）；② `PRD-M1` F-01「关联 schema」**漏 `PD-01 task`**（而 `PD-04.raised_by_task_id` 是 NOT NULL FK→`task`，不建任务落不了待补项）；③ `test-M1.md` `TC-I-M1-002` 引用写「`schema.md` **PD-05**（UNIQUE `idempotency_key`）」——该约束实际在 **MD-12** `research_proposal`（行号 L302 正确、表号错）；④ `server/README.md` 模块清单 `task-runner` 行功能点写「F-02 F-04 F-05 F-06」**漏 F-01**（阶段3 覆盖 F-01~F-06）；⑤ `PRD-M1` F-03「关联 schema」**只列 `MD-12`、漏 `PD-05 opportunity_status_log`**——而 `test-M1.md` `TC-D-M1-007` 把 `PD-05` 的机会外键归给 **F-03**，种子 `LG-003`/`LG-004` 两条也写着「PM 提交研究建议（OPP-012 / OPP-010）」、正是 `candidate→submitted`（即 F-03 人工节点完成的那一步） | ⬜ **待裁决（2026-09-19 由阶段3 实施发现并登记，未擅自改上游）**：五处均为**登记 / 引用失真**，不影响表结构；修法明确、代价低（改文字），但按项目惯例**先登记等点头**，不擅自改上游。**其中 ④ 已由实施侧回写修复**——`server/README.md` 属 `server/` 自己的索引（非 `docs/` 锁定件），F-01/F-02 落地时已补成「F-01 F-02」、本次补至「F-01 F-02 F-03」；①②③⑤ 仍待裁决 | ④ 已修；①②③⑤ 未落地（等裁决）。实施侧已按「**以 `PRD-M1` 功能点范围为准 + 表结构以 DDL 为准**」推进：F-01 已按需写 `PD-01` 与 `LNK-04`；F-03 已按 `TC-D-M1-007` 的归属写 `PD-05`（经 F-10 单一写入面），并在 `server/task-runner/README.md` 登记本项 |
| Q-15 | 种子 `T-1023`（`hva_followup`）的**追问链不自洽**：`parent_task_id='T-1021'`，但同一行的 `trigger_basis` 写「PM 于 2026-09-18 09:11 **在 R-007 上**提交追问」，且 `LNK-04` 的 `output` 关联是 `R-007`——而 `T-1021` 产出的是 `R-006`（`LK-TO-006`）。即「追问的原研究」与「`parent_task_id` 指向的原任务」对不上 | ⬜ **待裁决（2026-09-19 由阶段3 实施发现并登记，未擅自改种子）**：两个方向——① **改种子**（`T-1023.parent_task_id` 改指产出 `R-007` 的任务，或修正 `trigger_basis` / `task_object` 三者之一使自洽）；② **保持种子不动**，由 F-05 实现按「**原研究 → `LNK-04` 的 `output` 反查产出任务**」为主口径、`parent_task_id` 只作补充（**当前实现按 ②**，并把「两者不一致」显式回报为 `origin_task_mismatch` 而非静默取一） | 待 F-05 落地后补实测（`task-runner` 的追问件）；裁决后按 ① 需改种子单文件 |
...
| `../07-decisions/ADR-001.md` `ADR-002.md` `ADR-003.md` | 三项裁决的正式决策记录（问题 / 备选 / 最终 / 权衡 / 影响面 / 回滚代价） | 落地细节与「将来反悔难度」的依据 |
...
| `tech-stack.md` | 承接本文件 §12 Q-01（表名前缀）、Q-02（数据库方言）的去处；并回填 D1 实测结论（TS-11 / TS-14） | ✅ **已建**（2026-09-19，v1.2） |
