...
| Q-08 | `PD-06.ref_object_type`（走 `dict:OBJECT_TYPE`，现四项＝`goal`/`opportunity`/`research`/`proposal`）**装不下三类上下文对象**——`background`（MD-04 背景条目）/ `source`（CFG-01 来源）/ `existing_evidence`（EXT-02 证据）在 `dict:OBJECT_TYPE` 无取值，而 `ADR-001` §3 第 3 条要求 PD-06「记下注入了哪几条对象」（背景尤甚） | ⬜ **待裁决（2026-09-19 由 F-12 实施发现并登记，未擅自改口径）**：两个方向——① **扩 `dict:OBJECT_TYPE`**（增 `background`/`source`/`evidence` 三个 `dict_item`，PD-06 逐条落行，满足 ADR-001 的可回溯粒度）；② **维持四项**（三类只装配、不落 PD-06 行，可回溯粒度退到「注入了哪些类型」）。**当前实现按 ②**（`server/shared-context/index.js` 的 `CONTEXT_OBJECT_TYPE` / `CONTEXT_WITHOUT_OBJECT_TYPE`，装配结果打 `no_object_type_q08` 显式标出，已写入 `test-f12.mjs` 断言） | 待 PM / 我裁决；裁决后改一张映射表即可，无 DDL 变更 |
