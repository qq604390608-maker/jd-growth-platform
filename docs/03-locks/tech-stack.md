...
| 编号 | 待确认事项 | 谁提供 | 阻塞什么 |
| ---- | ---- | ---- | ---- |
| TS-10 | **Workers AI 具体模型选型**：F-20 五查需强推理且须「敢说不足以判断」，须实机验证 | 技术方 + PM | 推理质量与「不编造」红线的达成。**2026-09-20 进展（仍未关闭）**：客户端封装已收编 —— `server/agent-orchestrator/llm-client.js`（56 断言全绿）+ `wrangler.toml` 的 `[[ai]]` binding；`MODELS` 常量池给出了**确切 ID 形态**（`@cf/<厂商>/<模型>`，如 `@cf/deepseek/deepseek-v4-flash`），解掉了原候选池 `*` 不确定 ID 的问题。**但默认值仍是待验证暂定**：用户裁决「先不定，跑实机验证」→ 须用 `external-deps.md` §6 的 mock 响应实测「敢说不足以判断」后方可钉死并关闭本项 |
