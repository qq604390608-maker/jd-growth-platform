...
| `03-locks/` | 三项锁定（schema 表结构 / external-deps 外部依赖 / tech-stack 技术栈） | ✅ 三项齐备（schema **v1.8**、external-deps **v1.2**、tech-stack **v1.4**；各含待确认清单） |
...
- `03-locks/` 的 `external-deps.md` 被 `../db/`（`source_registry` / `tool_registry` / `tool_permission` 种子）与 `../prototype/mock/`（mock server 契约）引用；其 §7 待确认清单（21 条登记，截至 2026-09-21 **未关 11 条**＝T-01~T-10 ＋ T-22，全部等对接方提供真实契约；技术侧可决的 T-20/21/23/24/25/26/27 与文档类 T-30/31/32 均已关闭；未关项含 5 条 demo 占位假性就绪＝T-01~T-05）关闭前不宜视为可实施
