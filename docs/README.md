...
- `03-locks/` 的 `external-deps.md` 被 `../db/`（`source_registry` / `tool_registry` / `tool_permission` 种子）与 `../prototype/mock/`（mock server 契约）引用；其 §7 待确认清单（21 条登记，截至 2026-09-20 **未关 18 条**——T-30 已解决、T-23/T-32 已决，T-24 改窄仍列；未关项含 5 条 demo 占位假性就绪）关闭前不宜视为可实施
