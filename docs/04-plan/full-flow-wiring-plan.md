...
| 反向清单 | 登记 `./README.md`（04-plan 文件清单）；引用于 `dev-plan.md` 阶段 4「接线进展」；本文件为 F-33~F-41 的**编号与范围依据** |
...
| **F-41** | **`hva_followup` 追问执行体接线**（**全流程最后一个断点**） | 4.1 | ✅ **已落地 2026-09-22**（用户「整完整个开发、需确认的走默认」授权）。**问题**：`hva_followup` 不在 `WIRED_TASK_TYPES` 内 → 追问任务建行后五步恒 `pending`、永不被 cron 选中，发起追问后**永远 `0 / 5 步`**；目标配置 / 发现 / 机会 / 提建议 / HVA 研究 / 研究结果 六环均已打通，只差追问这一环。**修法**：追问与研究**同构**（五步体共用），故不新建文件——① `research.js` 增 `FOLLOWUP_TASK_TYPE` / `intakeFollowup`（**复用 F-22 既有导出**，不复制类型常量）+ `RESEARCH_TASK_TYPES` + `RESEARCH_STEP_COUNT_OF`（**派生自 `TYPE_STEPS`**）；② `WIRED_TASK_TYPES` 由 `discovery/hva_research` 增为 `discovery/hva_research/hva_followup`（分派 / 选取 / 自愈补扫 / queue 路由四处**同取这一份真源**，自动生效）；③ 步 ① 追问**额外**走一次 F-22 承接判定（沿用 / 补查 / 未决）并如实写进 `done_part`，`hva_research` **完全不走**该路径（用例有正反两侧）。**零 schema 变更**。**oracle 翻转**：`test-f33` 的 ④「hva_followup 仍走占位」与 ⑩「不被选取」两处原把「未接线」固定为断言，随接线**如实翻转**（99 → **101 断言全绿**）；新增 `test-f41.mjs`（**30 断言全绿**：静态派生 + 步 ① 承接 + 反例 + **生产同形全链路**（只启用 CDP/HJE/MKT/ACT → 追问五步跑满 → 报告落在**新**研究、原研究逐字段未被覆盖）+ 越界步号边界）。**边界如实登记**：仍未接线者只剩 `goal_check`（2 步目标口径检查任务，仍走 `delegateToAgent` 占位） |
