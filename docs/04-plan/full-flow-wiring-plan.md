...
| **F-35** | 取号原子化（`id_sequence` 表） | 4.2 | ⏸ **待确认后开工——已评估「需改锁定件」**（本项**不是**零 schema 变更：新增 `id_sequence` 表 → ① `docs/03-locks/schema.md` 现为 **v1.3「36 张表」的字段级唯一转换口径**，须改版并按项目流程 bump 版本 + 旧版进 `.trash/` + 比对 SHA + 同步下游钉版；② `db/migrations/0001_init.sql` 是**单一 DDL 文件**，其「36 张业务表」声明同改；③ 全仓「36 张表」叙述多处同步。另：本节原文写「`opportunity` / `evidence` / `query_record` 统一改走它」，但**实测现有取号面为 15 个函数 / 4 个模块**（`OPP`/`LNK-03` 关系号/`FD`/`CB`/`BP`/`T`/`R`/`PROP`/`GAP`/`Q`/`BL`/`PD-07` 消息号…；其中 `evidence` 的号是**派生**自 `query_id`、本无竞态），故**范围口径需先定**（「只改 3 张表」还是「全部命名空间」），且租约/取号的**写面归属**（落哪个模块的 store）也需一并定。**按惯例未擅动锁定件、未开工**） |
