...
> 一句话结论：**计划内 32 个功能点（F-01~F-32）与三项缺陷已全部写完并合入本地 `main`；门禁 B 部署已收口；门禁 C 外部契约已于 2026-09-21 依 ADR-004 自拟收口（契约基准 v1）——三道门禁全部关闭。**
...
>
> **2026-09-21 收口依据（ADR-004）**：项目确认**无外部对接方**，未决的 T-01~T-10 + T-22 共 11 条由我方以「平台方 + 对接方」双重角色**自拟答案并冻结为契约基准 v1**（`external-deps.md` 升 v1.3）；自造值允许进断言（标注「基准 v1」），「真实返回」红线改述为「来源可追溯」。真对接方出现时**走 ADR 修订替换**，不直接改值。原对外提问稿 `external-intake-questions.md` 转为「自拟契约记录」留档。

### 3.1 数据源类（`T-01`~`T-10` **✅ 全部自拟收口（2026-09-21，ADR-004）**——下表留档，自拟内容见 `external-deps.md` v1.3 §7 各行处置结果）
...
| T-22 | ✅ | 选定自建 MCP 客户端（DS-06）；协议细节已随 ADR-004 冻结为**契约基准 v1**（五面实现定案，见 `external-deps.md` v1.3 §7 T-22 行与 `mcp-client.js`） |
...
> **当前纪律（2026-09-21 起，依 ADR-004 改述）**：外部调用走**契约基准 v1**（原 mock/demo 契约转正），基准值**允许进断言（标注「基准 v1」）**；查询返回须**来源可追溯**（追溯到基准 v1 的约定），不得凭空生成。换言之：**平台可运行、任务可留痕、研究链路在基准 v1 上真实跑通**；真对接方出现时按 ADR 修订替换契约（届时 bump 基准 v2 + 种子与断言同步）。
