...
| 事实来源 | **本地实测**（全量回归 32/32 套件、mock server 冒烟、前端六页 HTTP 交付），非推断 |
| 适用范围 | 京东超市单一频道，不做全局 |
...
| 全量回归 | ✅ | **32/32 套件通过，内部手动断言合计 2194 条** |
...
| 编号 | 状态 | 说明 |
| ---- | ---- | ---- |
| **TS-10** | ⬜ **最大风险** | Workers AI **具体模型选型**须实机验证。它直接决定 F-20 第⑤项「信息是否足以支持判断」——模型必须**敢于说「不足以判断」**，这与模型对齐倾向强相关，须用 `external-deps.md` §6 的 mock 响应做回归 |
| TS-11 | 🟡 | 事实面已齐（D1 长度**一律不强制**；`CHECK` 可用、按**字符**计、**不拦 NULL**），但**策略仍待裁决**（应用层校验 or 库级 `CHECK`）。若裁决库级 `CHECK` 须写进 `0001` 迁移 |
...
| 5 | TS-11：`varchar(n)` 长度用应用层校验还是库级 `CHECK` | 技术方 | 是否需改 `0001` 迁移 |
