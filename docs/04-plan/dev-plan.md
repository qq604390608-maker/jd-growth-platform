...
- **F-35 已获裁决、待实施（2026-09-22）**：**范围＝「任务 / 研究 / 机会」三条最热号（选项 c）**，且**用户同意现在改锁定件**（`docs/03-locks/schema.md` 的「36 张表」→ 37、`db/migrations/0001_init.sql` 表数声明、全仓叙述同步，走 bump 版本 + 旧版进 `.trash/` + 比对 SHA + 同步下游钉版的全套流程）。实测取号面共 15 个函数 / 4 个模块，本轮只收敛最热三条，其余登记待办。
