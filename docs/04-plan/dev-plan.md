...
- **新增工作项 F-33~F-41（F-33~F-36 于 2026-09-21 登记；F-37/F-38 于 2026-09-22 由「独立工作项」转正并同批落地；F-39/F-40 于 2026-09-22 由「独立工作项」转正并同批落地；**F-41 于 2026-09-22 新增＝`hva_followup` 追问执行体接线，全流程最后一个断点**）**：全流程打通的四期方案见 `./full-flow-wiring-plan.md`（断点清单 + 裁决记录 + 编号映射）。区间裁决＝**期 1 + 期 2**；**F-33**＝M4 执行体接线（`server/task-runner/research.js` 五步 + `executor.js` 按 `task_type` 分派 + `index.js` 路由 + `wrangler.runner.toml` 加 `[ai]` ＋ 自愈补扫）、**F-34**＝步骤配额 + 去重 + 超时（零 schema 变更）、**F-35**＝取号原子化（`id_sequence`，**唯一非零 schema 变更项**：v1.9 改锁定件、表数 36 → 37）、**F-36**＝轮询与执行体的顺序/异常隔离、**F-37**＝`ended_at` 脏态修法（裁决＝resume 清空）、**F-38**＝F-19 查证计划避开未启用来源（裁决＝是）。另有两项**独立工作项**（未擅自改，已登记）：F-03/F-04 幂等守卫前移（消除孤儿任务 + 孤儿研究壳）、`followup.js` `research_status` 由 item_name 改回字典 item_code。期 3（真 Queues）与期 4（本地一键 e2e）本次不在范围内。
...
- **F-34 已落地（2026-09-21，待提交）**：期 2.1 加固版＝**步数配额 + 同 tick 去重 + 单步超时**（零 schema 变更）。新增 `server/task-runner/tick-guard.js`（**纯计算件**：零数据库访问、零 SQL、零写）＋ `test-f34.mjs`（**56 断言全绿**，进 CI）。要点：配额与超时分别**派生自** `TYPE_STEPS`（`./step-plan.js`）与 `DEFAULT_TIMEOUT_MS`（`../tool-executor/index.js`），**不复制第二份口径**；**超时不是失败**——只放弃等待、不落 `PD-03`、不改 `task_status`、不落 done，步骤保持 `active` 交下个 tick 重试（「慢」与「失败」必须分开，后者才走 F-26）；**同 tick 去重兼作死循环闸**（超时步骤仍是 `active`，无去重会被下一轮重查再次选中 → 同 tick 无限重试）；`exhausted` 语义＝「配额已尽**且仍有待执行的 active 步**」（实现上要求**先选批再判配额**）。堵的是 P0-2：一个 tick 跑完整批（12 任务 × 5 步 = 60 步）→ 跨 tick 重叠 → 同一 `active` 步被并发执行 → 产物翻倍（93 条重复机会的成因），配额把重叠窗口由 60 步缩到 5 步。**边界如实登记**：**不是真独占**——并发 tick 仍可能同时选中同一步；真独占需 2b 租约（`PD-02` 加列，已否决）或期 3 真 Queues 的 `max_batch_size = 1`。`F-35`（取号原子化）**已于 2026-09-22 落地**——本项**不是**零 schema 变更：新增 `id_sequence` 表使 `docs/03-locks/schema.md` **升 v1.9、表数 36 → 37**（**业务表仍 36 张**），`db/migrations/0001_init.sql` 表数声明与全仓叙述同批同步；**实测现有取号面为 15 个函数 / 4 个模块**（远超方案原文写的 3 张表），本轮只收敛最热三条、**其余 12 个登记待办**（`schema.md` §12 **Q-18**）。详见 `./full-flow-wiring-plan.md` 编号映射表 F-35 行与本文件下方 F-35 条目。
- **F-36 已落地（2026-09-21）**：期 2.3＝**调度相位异常隔离**（零 schema 变更）。`server/task-runner/index.js` 把「裸 await 三连」抽成 **一个 tick 三相位**，新增导出 `TICK_PHASES`（顺序真源 `due > heal > work`）、`runPhase`（单相位 catch）、`EMPTY_WORK`（失败时**键齐的空形态**）、`runTick`（可测本体，**相位与 `onError` 均可注入**），`scheduled` 退化为一行薄壳；＋ `test-f36.mjs`（**45 断言全绿**，进 CI）。堵的是 **P1-1**：原实现 `runDueDiscoveryCalls` 排在执行体之前且**无 try/catch**，任一目标抛错 → 整个 `scheduled` reject → 本 tick 所有 `active` 步永不执行且**每分钟重演**；现任一相位抛错**只影响该相位**、其余照跑，失败**如实回报**在 `phases.<相位>.{ok,why}` 并呼叫 `onError`（默认 `console.error`，供 `wrangler tail` 观测）——既不炸整轮、也不静默吞错。**顺序刻意不重排**（取方案原文的第二个选项）：原文举的失效模式是抛错、隔离已足够，而重排会失去**同 tick 流水线**（本轮新建 / 自愈补回的 `active` 步在同一 tick 被消费），`test-f36` ④ 以「一个 tick 内跑满五步 → 任务 done」固定这条收益。**纯控制流改动**：不新增 import（静态断言 import 仍恰 5 条）、不改 schema、不改上游语义。**边界如实登记**：`queue()` 的**逐消息隔离不在本项范围**（真 Queues 未接，属 P1-4，接入后单独处理）。
- **F-37 已落地（2026-09-22）**：`ended_at` 脏态修法，**用户裁决＝「resume 清空」**（零 schema 变更）。`server/tool-executor/task-state.js` 的 `setTaskStatus` 增 `clear_ended_at`＝**唯一显式清空口**（实现仍是**单条 `CASE` SQL**，故 `test-f26` 的「`UPDATE task` 恰 2 条」静态断言**逐字未动**），`server/task-runner/recovery.js` 的 `resumeTask` 用它。堵的是「受阻侧两个写入方口径不一（F-26 对 `blocked` 留空 / 执行体 `catch` 落时刻）→ 恢复后成「`running` + `ended_at` 非空」，与锁定列口径『任务结束时点；**进行中为空**』相悖」；线上同源可复现（T-0026 `ended_at=15:26` vs T-0029 `null`）。`test-f06` 47→**51 断言全绿**（脏态夹具前置 + 恢复后清空 + **反例「未显式声明清空时沿用 `COALESCE` 保留旧值」**）。「`blocked` 不落值」方向**未采纳**，受阻侧两写面保持原样。
- **F-38 已落地（2026-09-22）**：F-19 查证计划避开未启用来源，**用户裁决＝「是」**（零 schema 变更）。`server/agent-orchestrator/research-start.js` 收 `available_sources`：计划只用「**已声明 且 可用**」的来源并**逐步骤收窄 `data_sources`**（**只丢步骤是不够的**——某步只因 MKT/ACT 可用而保留时，若 `data_sources` 仍写着 PIM，执行体照计划取数就会打到未接入来源），排除项**如实登记**为 `plan_excluded_sources` / `plan_excluded_checks` / `availability_note`；`server/task-runner/research.js` 经 M5 `listTools` 现读可用来源（本体仍**零裸 SQL**）后注入，并把排除项写进步 ① `done_part` 与返回体。**未注入 → 口径不变（不过滤）**；既有「`[]`＝不过滤」语义由 `allowEmpty` **显式开启**而非静默改掉。`test-f19` 94→**108**、`test-f33` 88→**103** 断言全绿（新增 ⑫「生产同形」：只启用 TOL-01/04/09/11 → **PIM 不被查询、零 `PD-03`、五步跑到 `done` + 报告落库**——这正是线上 T-0031 被卡住的那个形态）。**边界/待裁决如实登记**：只判「该来源有无已启用工具」，权限与接入态仍由 M5 前置守卫兜底；①「零可用来源时是否应直接受阻」→ **用户裁决＝保持现状**（不主动置 blocked，依赖 M5 守卫兜底；STATE_LIMITATION 路径本就无来源，误阻风险高）②报告「其他解释与限制」是否纳入来源排除说明 → **用户裁决＝纳入（已落地）**：`research.js` `buildLimitsText` 追加「来源排除」段（与 `done_part` / `plan_excluded_sources` 口径一致），`test-f33` ⑫ 加断言 e6_limits 含被排除来源、且无排除时不出现该段。
- **F-35 已落地（2026-09-22）**：取号原子化，**用户裁决＝范围取「任务 / 研究 / 机会」三条最热号（选项 c）＋ 同意现在改锁定件**。**已改锁定件**：`docs/03-locks/schema.md` **v1.8 → v1.9**（新增 **CFG-09 `id_sequence`**；表数 **36 → 37**＝业务表仍 36 张（MD 14 ｜ PD 7 ｜ CFG 8 ｜ LNK 4 ｜ EXT 3）＋ 1 张**运行期基础设施**计数器表；旧版归档 `.trash/schema.md-v1.8.md`，SHA 4afb1679ad12）＋ `db/migrations/0001_init.sql` 表数声明同改（**37 个 `CREATE TABLE`**，头部钉 `schema.md v1.9`）。落地件＝`server/shared-context/id-sequence.js`（**该表唯一写入面**：热路径**一条** `UPDATE ... RETURNING` 完成「自增 + 取值」；冷路径自愈种子从消费方传入的既有编号列表抬到「库内实际最大」，故线上既有数据**不需要补种**；`healSequence` 显式修漂移、`max()` upsert、**只抬不降**、幂等；**热路径 0 次回读业务表**——性能纪律）。消费方＝`task-runner/step-plan.js`（`nextTaskId` / `nextResearchNo`）与 `agent-orchestrator/opportunity.js`（`nextOpportunityId`），**调用面与返回形态逐字未变**（F-16 / F-05 / F-01~F-06 断言**一字未改**）、号形态（前缀 + 位宽）与命名空间**只此一份**（消费方不再持正则）。`test-f35.mjs` **26 断言全绿**并进 CI（含并发 6 次互不相同、热路径 0 次回读、只抬不降幂等、`schema.md` 声明表数 ↔ DDL `CREATE TABLE` 实数漂移守卫）。**线上既有库**补建走一次性入口 `db/ops/2026-09-22-id-sequence.sql`（`CREATE TABLE IF NOT EXISTS`，幂等、无破坏性语句）＋ `ops-id-sequence.yml`（`0001_init.sql` 已被 `d1_migrations` 记为已应用，D1 **不会重放**）。**边界如实登记**：① 其余 **12 个取号函数 / 4 个模块仍为读后写**、同样存在 P0-3 竞态，本轮未收敛（`schema.md` §12 **Q-18** 待办；收敛无需再改 DDL）；② 只保证「经本表取号者之间」不撞号，**旁路插入**（运维 SQL / 手工补数）仍可能撞号——自愈种子可收回漂移，但与首次取号并发时仍有窄窗口、**不承诺消除**。**遗留（未擅动）**：`tech-stack.md` / `external-deps.md` 里的「36 张表」交叉引用**未同步**——二者自身是锁定件，改动须各自 bump 版本 + 归档 + 同步下游钉版，本项未擅自扩大范围，已登记待用户点头。
- **F-39 已落地（2026-09-22）**：**F-03/F-04 幂等守卫前移**，消除孤儿任务 + 孤儿研究壳（零 schema 变更）。原 ⑤ 的建议级守卫 `markProposalTriggered` 因 `task_id` 入参必须是**已存在**任务（`MD-12.triggered_task_id` 外键），只能排在「建任务 / LNK-04 / 建研究壳 / PD-06」**之后**——重复调用虽最终报错，却**已留下孤儿任务 + 孤儿研究壳**（`triggered_task_id` 只指回最后一次那条），即线上 `PROP-001 → T-0026 + T-0029` 双任务的同形机制。修法＝`proposal.js` 新增**只读预检** `ensureProposalNotTriggered(db, proposal_id)`（只判「是否已触发过」、**不需要 `task_id`**，故可排在最前），`createHvaResearchTask` **在建任何行之前**先调它；`markProposalTriggered` 退为只落 `triggered_task_id` 的写。**判据与原来一致**（仍报「已触发任务」，不引入第二套口径）。`test-f04` **A44 由「现状 4 行」翻转为「期望 3 行」**＋新增 A45（孤儿任务 0）/ A46（`triggered_task_id` 指回**首次**那条）＝ **66 断言全绿**；`test-f03` 新增 ⑩ 段＝ **90 断言全绿**。
- **F-40 已落地（2026-09-22）**：`followup.js` 建追问研究壳时 `research_status` 由 item_name「研究中」改回字典 `RESEARCH_STATUS` 的 **item_code `running`**（`varchar(16)` 无 CHECK，**库级拦不住**，只能靠断言守），与 F-04 `hva.js` 建壳**同口径**。`test-f05` 新增 **A34/A35** 正反两侧（值域**取自库**的 `dict_item` + 反例「不得写 item_name」）＝ **59 断言全绿**。零 schema 变更、一行改动。
- **F-41 已落地（2026-09-22）：`hva_followup` 追问执行体接线——全流程最后一个断点消除**（零 schema 变更）。此前 `hva_followup` 不在 `WIRED_TASK_TYPES` 内，追问任务建行后五步恒 `pending`、永不被 cron 选中，发起追问后**永远 `0 / 5 步`**；而目标配置 / 发现 / 机会 / 提建议 / HVA 研究 / 研究结果 六环均已打通。**修法**：追问与研究**同构**（五步体共用），故不新建文件——`research.js` 增 `FOLLOWUP_TASK_TYPE` / `intakeFollowup`（**复用 F-22 既有导出**，不复制类型常量）、`RESEARCH_TASK_TYPES`、`RESEARCH_STEP_COUNT_OF`（**派生自 `TYPE_STEPS`**）；`WIRED_TASK_TYPES` 增为 `discovery / hva_research / hva_followup`（分派 / 选取 / 自愈补扫 / queue 路由四处**同取这一份真源**）；步 ① 追问**额外**走一次 F-22 承接判定并如实写进 `done_part`，`hva_research` **完全不走**该路径（正反两侧都锁死）。**oracle 翻转**：`test-f33` 的 ④/⑩ 两处「hva_followup 未接线」现状断言随接线翻转为期望（99 → **101 断言全绿**）；新增 `test-f41.mjs`（**30 断言全绿**，含**生产同形全链路**：只启用 CDP/HJE/MKT/ACT → 追问五步跑满 → 报告落在**新**研究、原研究逐字段未被覆盖）。**线上实测**：追问任务 T-0041 由 `0 / 5` 跑到 `5 / 5 done`，新一轮研究结果落新研究行、原研究原样保留。**边界**：仍未接线者只剩 `goal_check`（2 步目标口径检查任务，走 `delegateToAgent` 占位）。
